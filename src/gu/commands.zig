const types = @import("pspgutypes.zig");

// See https://ziglang.org/documentation/master/#Tagged-union
// at section `test_tagged_union_with_tag_values`.
// Check pspsdk/src/gu/doc/commands.txt for the original version
pub const GuCommand = union(enum(u8)) {
    // Dec Hex Name        Description
    // --- --- ----        -----------
    //
    // 0   00  NOP         No Operation
    nop: packed struct(u24) {
        zero: u24 = 0,
    } = 0,
    //
    ...
    clear: packed struct(u24) {
        enable: bool,
        zero_b1_7: u7 = 0,
        flags: types.GuClearFlags,
        zero_b11_23: u13 = 0,
    } = 211,
    //
...
pub fn convert_to_u32(cmd: GuCommand) u32 {
    const raw_command = RawGuCommand{
        .payload = switch (cmd) {
            inline else => |payload| @bitCast(payload),
        },
        .op_code = @intFromEnum(cmd),
    };

    return @bitCast(raw_command);
}

const RawGuCommand = packed struct(u32) {
    payload: u24,
    op_code: u8,
};

const TruncatedFloat = packed struct(u24) {
    truncated_float: u24,

    pub fn make(value: f32) TruncatedFloat {
        return TruncatedFloat{
            .truncated_float = truncate_float(value),
        };
    }
};

pub fn truncate_float(value: f32) u24 {
