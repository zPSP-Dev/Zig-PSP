...
export fn cbThread(args: libzpsp.types.SceSize, argp: ?*anyopaque) c_int {
    ...
    var status = libzpsp_loadexec.sceKernelRegisterExitCallback(cbID);
...
pub fn enableHBCB() void {
    const threadID: i32 = libzpsp_threadman.sceKernelCreateThread(@ptrCast("zig_callback_updater"), @bitCast(@intFromPtr(&cbThread)), 0x11, 0xFA0, @intFromEnum(pspthreadman.PspThreadAttributes.PSP_THREAD_ATTR_USER), null);
    if (threadID >= 0) {
