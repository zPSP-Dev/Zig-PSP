...
pub fn allocVramRelative(width: u32, height: u32, format: GuPixelMode) ?*anyopaque {
    var res = vramOff;
    vramOff += getMemSize(width, height, format);
    return @intToPtr(?*anyopaque, res);
}

//Allocate a buffer of VRAM in VRAM-Absolute pointers (0x04000000 start)
pub fn allocVramAbsolute(width: u32, height: u32, format: GuPixelMode) ?*anyopaque {
    return @intToPtr(?*anyopaque, @ptrToInt(allocVramDirect(width, height, format)) + @ptrToInt(sceGeEdramGetAddr));
}
